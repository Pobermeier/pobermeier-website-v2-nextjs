...
const useEmailPlaceholders = (): void => {
  ...
  useEffect(() => {
    ...
    })(document.querySelectorAll('.email-placeholder'), config.urls.email);

    const emailButton = document.querySelector('#email-button-placeholder');

    if (emailButton)
      emailButton.innerHTML = `<a class="button is-rounded is-secondary gtm-tracked-link" href="mailto:${config.urls.email}" id="mail-button">
    ...
    emailIcon.href = 'mailto:' + config.urls.email;
